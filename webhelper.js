...
function nextTurn(){
	...
	if (turn <= turns.length) {
		...
		while(message<messages.length){
			message++;
			if(messages[message][0]==turn){
			let messageText = messages[turn][1];
			alert(messageText);
			}
		}
...
function removeCaptures(captures,color,capColor){
	...
	if (turn == 76|| turn==74){
		alert("Problem: "+turn+" Captures: "+captures+" Color: "+color+" Capture Color: "+capColor);
	}
